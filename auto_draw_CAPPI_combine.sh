...
for((i=$startimestamp0;i<=$startimestamp1;i+=300))
do
	currenttime=`date -d "@$i" "+%Y%m%d%H%M%S"`
	echo "current time:" $currenttime
    python3 /mnt/e/Doksuri/scripts/Radar/pycwr/CAPPI_overlay_terrain_combine.py /mnt/f/Doksuri/data/RADAR_S $currenttime /mnt/e/Doksuri/fig/Radar_CAPPI_combine dBZ 1500
done
